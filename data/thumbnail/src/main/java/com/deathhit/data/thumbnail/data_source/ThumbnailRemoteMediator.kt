        ...
        return try {
            ...
            appDatabase.withTransaction {
                if (loadType == LoadType.REFRESH) {
                    appDatabase.breedDao().clearAll()
                    appDatabase.breedThumbnailRefDao().clearAll()
                    appDatabase.thumbnailDao().clearAll()
                ...
                appDatabase.breedThumbnailRefDao().insertOrReplaceAll(breedRefs)
                appDatabase.breedDao().insertOrReplaceAll(breeds)

                val thumbnails = response.map {
                    ThumbnailEntity(it.id, it.url)
                }
                appDatabase.thumbnailDao().insertOrReplaceAll(thumbnails)
            }
